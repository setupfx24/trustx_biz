...
export async function downloadTradeStatementPdf(
  ...
  doc.text(
    `Generated: ${formatWhen(new Date().toISOString())} (local time)`,
    margin,
    y,
  );
  y += 5;
  ...
  if (totalComm !== 0 || totalSwap !== 0) {
    doc.text(
      `Commission (sum): ${fmtUsd(totalComm)}  ·  Swap (sum): ${fmtUsd(totalSwap)}`,
      margin,
      y,
    );
    y += 4;
  ...
  autoTable(doc, {
      ...
      [
        ...
        },
        {
          ...
          styles: {
            fontStyle: "bold",
            halign: "right",
            textColor: totalPnl >= 0 ? [0, 120, 80] : [180, 0, 50],
          },
        },
      ],
    ],
    ...
    didDrawPage: (data) => {
      ...
      doc.text(
        "Trustx — for information only. Not tax or legal advice.",
        margin,
        doc.internal.pageSize.getHeight() - 6,
      );
    },
  ...
  doc.save(`trustx-trade-statement-${safeDate}.pdf`);
}
