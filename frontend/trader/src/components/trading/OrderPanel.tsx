        ...
        <div className="flex flex-col items-end shrink-0">
          <div className="flex items-center gap-1">
            <span
              ...
              style={{ color: marketStatus.isOpen ? '#035eeb' : '#f57c00' }}
            >
              {marketStatus.isOpen ? 'OPEN' : 'CLOSED'}
            </span>
            {isConnected ? (
              <Wifi size={isTradingTerminal ? 11 : 12} className="text-[#035eeb]" />
            ) : (
        ...
        >
            ...
            {(['market', 'pending'] as const).map((t) => (
              <button
                ...
                style={{
                  ...
                  borderBottom:
                    orderTab === t
                      ? `2px solid ${isTradingTerminal ? '#2962FF' : '#035eeb'}`
          ...
          <div className={clsx('grid grid-cols-2', isTradingTerminal ? 'gap-1.5' : 'gap-2')}>
             ...
             <button
                ...
                className={clsx(obPad, 'rounded-lg flex flex-col items-center justify-center transition-all duration-150 active:scale-[0.98]')}
                style={{
                  background: side === 'buy' ? 'rgba(3, 94, 235,0.15)' : 'var(--bg-secondary)',
                  border: side === 'buy' ? '1px solid #035eeb' : '1px solid var(--border-primary)',
                  color: side === 'buy' ? '#035eeb' : 'var(--text-secondary)',
                }}
             >
                <div className={clsx('font-bold uppercase tracking-wider', isTradingTerminal ? 'text-[10px] mb-0' : 'text-sm mb-0.5')}>Buy</div>
                <div className={clsx('font-mono font-bold', isTradingTerminal ? 'text-[13px]' : 'text-[15px]', side === 'buy' && 'text-[#035eeb]')}>{tick ? tick.ask.toFixed(digits) : '---'}</div>
          ...
          {tpEnabled && (
            <div className="pt-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#035eeb] mb-1.5 block">Take Profit</span>
              <input
                type="number"
                value={takeProfit}
                ...
                className="w-full text-sm font-mono py-2.5 px-3 rounded-lg focus:outline-none bg-bg-secondary border border-[#035eeb]/30 text-[#035eeb]"
              />
            </div>
          )}
            ...
            <>
              ...
              <div className="rounded-xl p-3 space-y-2 bg-bg-secondary border border-border-primary">
                {[
                  ...
                  { label: 'Free Margin', value: fmtAccountMoney(freeMargin, isCentAccount(activeAccount)), color: !hasEnoughMargin ? '#ef5350' : '#035eeb' },
        ...
        {isTradingTerminal ? (
          <div className="shrink-0 border-t border-border-primary bg-bg-secondary px-2 pt-2 pb-2 space-y-1.5">
            ...
            <div className="flex items-center justify-between gap-1 px-1 text-[9px] text-text-tertiary">
              ...
              <span className={clsx('shrink-0 font-mono', hasEnoughMargin ? 'text-[#035eeb]' : 'text-[#ef5350]')}>
                Free {fmtAccountMoney(freeMargin, isCentAccount(activeAccount))}
              </span>
              <span
                className={clsx('shrink-0 font-mono', isConnected ? 'text-[#035eeb]' : 'text-[#f57c00]')}
            ...
            <button
              ...
              style={{
                background: side === 'buy' ? '#035eeb' : '#ef5350',
                color: '#fff',
                boxShadow: side === 'buy' ? '0 2px 12px rgba(3, 94, 235,0.2)' : '0 2px 12px rgba(239,83,80,0.2)',
              }}
            >
              {`${side === 'buy' ? 'Buy' : 'Sell'} ${selectedSymbol}`}
            </button>
            ...
            {presets.map((v) => (
              <button
                ...
                className={clsx(
                  ...
                  v === account.leverage
                    ? 'bg-[#035eeb]/15 text-[#035eeb] font-bold'
