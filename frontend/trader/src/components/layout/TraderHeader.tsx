...
import { TrustxWordmark } from '@/components/layout/TrustxWordmark';
  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 border-b border-border bg-bg-secondary">
      <div className="flex flex-wrap items-center gap-4">
        <TrustxWordmark href="/dashboard" textClassName="text-xl" />
          ...
          {NAV.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`px-3 py-1.5 text-sm rounded transition ${active
