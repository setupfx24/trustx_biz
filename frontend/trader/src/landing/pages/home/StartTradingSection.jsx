    ...
    <section className="section-padding bg-primary-bg">
      <div className="container-custom">
        <SectionHeader
          ...
          subtitle="Begin your trading journey with Trustx today"
        />
          ...
          {steps.map((step) => (
            <ScrollRevealItem key={step.number}>
              <div className="glass-card p-6 pt-8 h-full text-center relative overflow-visible">
                <div
                  className={`w-10 h-10 rounded-full ${step.bg} flex items-center justify-center mx-auto mb-4`}
                >
                  <span className={`${step.color} font-bold text-lg`}>
                    {step.number}
                  </span>
                </div>
                <div
                  className={`feature-icon ${step.bg} ${step.color} mb-4 mx-auto`}
                >
                  {step.icon}
                </div>
