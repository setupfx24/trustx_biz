...
async function proxy(
  ...
  const targetUrl = `${gatewayOrigin()}/${path}${req.nextUrl.search}`.replace(
    /([^:])\/\//g,
    "$1/",
  );
    ...
    if (isMultipart) {
      try {
        ...
        return NextResponse.json(
          {
            detail:
              "Could not read file upload. Try a smaller image (JPG/PNG) under 10 MB.",
          },
          { status: 400 },
        );
      }
  ...
  if ([301, 302, 307, 308].includes(res.status)) {
    ...
    if (location) {
      try {
        ...
        return NextResponse.json(
          { detail: "Gateway redirect failed" },
          { status: 502 },
        );
      }
    }
  ...
  /* Forward CORS headers from gateway — without these, browser preflight and
     cross-origin responses fail for callers like algo.trustx.biz. */
  for (const h of [
