...
async def upload_banner_image(file: UploadFile) -> dict:
    # Content-Type is attacker-controlled — kept as a fast cheap reject path
    # only. Magic-byte check below is the actual gate.
    ...
    declared_ext = (
        ...
        if file.filename and "." in file.filename else ".png"
    )
    allowed = (".png", ".jpg", ".jpeg", ".webp", ".gif")
    from packages.common.src.upload_safety import assert_matches, UnsafeUploadError
    try:
        kind = assert_matches(contents, declared_suffix=declared_ext, allowed_suffixes=allowed)
    except UnsafeUploadError as e:
        raise HTTPException(status_code=400, detail=str(e))

    out_name = f"{uuid.uuid4().hex}{kind.suffix}"
