...
When an Trustx user with book_type='A' opens, closes, or modifies a
...
async def forward_trade_close(
    ...
    pnl: "Decimal | float | int" = 0,
    closed_by: str = "USER",
    closed_at: str | None = None,
) -> dict:
    """Push an A-Book trade close to Corecen.

    Accepts Decimal upstream; only narrows to a JS-compatible Number at
    the JSON serialisation boundary inside _js_val(). Callers MUST NOT
    pre-cast Decimal → float — that loses precision before we even get
    here."""
    if not _is_configured():
        return {"skipped": True}
